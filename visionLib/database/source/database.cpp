...
bool ClientDataBase::insertClient(const std::string& Login,
...
bool PassDataBase::PassExists(const uint64_t PassID) {
  auto exist_sql_req =
      ("select exists(select id from client where pass.if = '" +
...
ClientDB ClientDataBase::getClient(const std::string login) {
  ...
  ClientDB result(row[0].as<int>(), row[1].as<std::string>(),
