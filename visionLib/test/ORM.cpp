...
class MyDataBase {
 ...
 public:
  ...
  int init(std::string username, std::string passwaord) {
    if (dbC.logIn(username, passwaord) == true) {
...
class MockDBInsert : public DataBase {
 public:
  MOCK_METHOD1(insertPass, uint64_t(const PassDB& Pass));
  MOCK_METHOD1(insertPassage, uint64_t(const PassageDB& Passage));
  MOCK_METHOD1(insertClient, uint64_t(const ClientDB& Client));
  MOCK_METHOD1(insertCompany, uint64_t(const CompanyDB& Company));
};
...
TEST(DBTest, InsertTest) {
  ...
  EXPECT_CALL(mcdb, insertPass(_)).Times((1)).WillOnce(Return(1));

  uint64_t res = db.insertPasslol(PassDB());
  EXPECT_EQ(res, 1);
}
